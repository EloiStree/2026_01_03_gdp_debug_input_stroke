class_name  InputUiValueChangedFromOptionButtonInt
...
@export var _target:OptionButton
...
func _ready() -> void:
	if _target==null:
		self.name = "UNASSIGNED TARGET BUTTON"
		return
	_target.item_selected.connect(_on_value_changed)
