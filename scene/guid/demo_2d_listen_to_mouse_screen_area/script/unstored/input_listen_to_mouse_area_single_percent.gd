class_name InputListenToMouseAreaSinglePercent
extends InputAbstractListenToMouseAreaTriggersSingle

@export var _area_border_percent:InputResourceMouseAreaBorderPercent
...
func push_in_percent_lrdt_mouse_position(mouse_position_percent_lrdt:Vector2) -> void:
	...
	if _previous_is_in_area != _current_is_in_area:
		if _current_is_in_area:
			...
			for trigger_name in _area_border_percent._exit_text_trigger:
				on_area_triggered_exit.emit(trigger_name)
		for trigger_name in _area_border_percent._enter_exit_trigger_boolean_name:
