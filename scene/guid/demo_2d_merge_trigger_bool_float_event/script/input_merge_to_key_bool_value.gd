class_name InputMergeToKeyBoolEvent
extends Node
...
static var _instance:InputMergeToKeyBoolEvent = null
...
static func _push_to_static_listener(key:String, value:bool) -> void:
	for callable in _callable:
		if callable.is_valid():
			callable.call(key, value)

static func get_instance() -> InputMergeToKeyBoolEvent:
	if _instance == null:
		_instance = InputMergeToKeyBoolEvent.new()
	return _instance
...
@export var _last_pushed_value:bool = false

func push_in(key:String, value:bool) -> void:
	if _strip_check:
		key = key.strip_edges()
